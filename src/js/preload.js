  ...
  (function () {
    ...
    window.fetch = async function (...args) {
      ...
      if (options && options.signal && url.indexOf('backend-api/conversation') > 0) {
        ...
        // invoke("onRecvChatGptMsg",{state:"REQUEST",text:JSON.stringify(options),index})
      }
      index += 1
      ...
      if (options && options.signal && url.indexOf('backend-api/conversation') > 0) {
        if (response.ok) {
          if (response.body && options) {
            invoke_api("onRecvChatGptMsg",{state:"START",index,text:""})
            const transformStream = new TransformStream({
              transform(chunk, controller) {
                ...
                invoke_api("onRecvChatGptMsg",{state:"IN_PROCESS",index,text})
              },
            });
            ...
            invoke_api("onRecvChatGptMsg",{state:"ERROR",index,text:error})
          }
        } else {
...
let currentWaiChatGptBotWorker
...
class WaiChatGptBotWorker{;
  ...
  static getCurrentInstance(){
    if(!currentWaiChatGptBotWorker){
      currentWaiChatGptBotWorker = new WaiChatGptBotWorker(+(new Date()))
    }
    return currentWaiChatGptBotWorker;
  }
  init(){
    console.log("===>>>> [WaiChatGptBotWorker inited]",this.id)
    invoke_api("WaiChatGptBotWorkerInit")
...
contextBridge.exposeInMainWorld('WaiApi', {
  onJsMsg: (action, payload) => {
    console.log('[onJsMsg]', action, payload);
    ipcRenderer.send('ipcMainMsg',action,payload);
  },
});
