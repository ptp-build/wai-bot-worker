...
export async function requestOpenAi(method: string, path: string, body: Record<string, any>, apiKey: string,
  ...
  if(stream){
    ...
    if(!customKey){
      await handleAskResult(
        body,chatId,msgId,msgDate,msgAskId,msgAskDate,authUserId,
...
export async function handleAskResult(
  ...
  // if(msgAskId){
  //   await new MsgBot(authUserId,chatId,authUserId).saveMsg()
  // }
  //
  // if(msgId){
  //   await new MsgBot(authUserId,chatId,chatId).saveMsg()
...
export async function requestOpenAiStream(
  ...
  const res = await requestOpenAi(
      'POST', 'v1/chat/completions', body, apiKey,chatId,msgId,msgDate,msgAskId,msgAskDate,authUserId,true) as Response;
